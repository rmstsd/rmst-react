...
export class TextService {
  ...
  measureText(text: string, parsedStyle: ParsedTextStyleProps): TextMetrics {
    ...
    if (textPath) {
      ...
      const height =
        Math.max(lineHeight, fontProperties.fontSize + lineWidth) + (lines.length - 1) * (lineHeight + leading)
      // if (dropShadow) {
  ...
  private wordWrap(text: string, parsedStyle: ParsedTextStyleProps): string {
    const { wordWrapWidth = 0, letterSpacing = 0, maxLines = Infinity, textOverflow } = parsedStyle
    const canvas = this.canvas
    const context = this.canvas.getContext('2d')
