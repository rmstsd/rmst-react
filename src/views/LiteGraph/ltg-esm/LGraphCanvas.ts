...
import { closeAllContextMenus, pointerListenerAdd, pointerListenerRemove } from './utils/GUI'
import { distance, isInsideRectangle, overlapBounding } from './utils/utils'
...
class LGraphCanvas {
        ...
        if (node && this.allow_interaction && !skip_action && !this.read_only) {
          ...
          if (!this.connecting_node && !node.flags.collapsed && !this.live_mode) {
            ...
            if (mClikSlot && mClikSlot_index !== false) {
              const alphaPosY =
                0.5 - (mClikSlot_index + 1) / (mClikSlot_isOut ? node.outputs.length : node.inputs.length)
  ...
  */
  ...
  checkDropItem({ dataTransfer, canvasX, canvasY }) {
  ...
  */
  ...
  convertEventToCanvasOffset({ clientX, clientY }) {
    const rect = this.canvas.getBoundingClientRect()
    return this.convertCanvasToOffset([clientX - rect.left, clientY - rect.top])
  }
  ...
  drawSubgraphPanelRight(subgraph, subnode, ctx) {
    ...
    ctx.font = '14px Arial'
    if (subnode.outputs)
      for (const output of subnode.outputs) {
    ...
    if (!node.flags.collapsed) {
      ...
      if (node.inputs) {
        for (var i = 0; i < node.inputs.length; i++) {
          ...
          ctx.fillStyle =
            slot.link != null ? slot.color_on ||
              this.default_connection_color_byType[slot_type] ||
              this.default_connection_color.input_on : slot.color_off ||
      ...
      if (node.outputs) {
        for (var i = 0; i < node.outputs.length; i++) {
          ...
          ctx.fillStyle =
            slot.links && slot.links.length ? slot.color_on ||
              this.default_connection_color_byType[slot_type] ||
              this.default_connection_color.output_on : slot.color_off ||
                this.default_connection_color_byTypeOff[slot_type] ||
                this.default_connection_color_byType[slot_type] ||
  ...
  renderLink(ctx, a, b, link, skip_border, flow, color, start_dir, end_dir, num_sublines) {
    ...
    if (this.ds.scale >= 0.6 && this.highquality_render && end_dir != LiteGraph.CENTER) {
      ...
      if (this.render_connection_arrows) {
        ...
        const posA = this.computeConnectionPoint(a, b, 0.25, start_dir, end_dir)
        const posB = this.computeConnectionPoint(a, b, 0.26, start_dir, end_dir)
        const posC = this.computeConnectionPoint(a, b, 0.75, start_dir, end_dir)
        const posD = this.computeConnectionPoint(a, b, 0.76, start_dir, end_dir)
  ...
  static onMenuAdd(node, options, e, prev_menu, callback) {
    ...
    function inner_onMenuAdded(base_category, prev_menu) {
      const categories = LiteGraph.getNodeTypesCategories(canvas.filter || graph.filter).filter(category =>
        category.startsWith(base_category)
      )
      const entries = []
      ...
      categories.map(category => {
        ...
        const index = entries.findIndex(({ value }) => value === category_path)
        if (index === -1) {
          entries.push({
            value: category_path,
            content: name,
      ...
      nodes.map(({ skip_list, type, title }) => {
        if (skip_list) return
  ...
  static onShowMenuNodeProperties(value, options, e, prev_menu, node) {
    ...
    for (const i in node.properties) {
      ...
      entries.push({
        ...
        }</span><span class='property_value'>${value}</span>`,
        value: i
      })
    }
    if (!entries.length) {
  ...
  showConnectionMenu(optPass) {
    ...
    function inner_clicked(v, options, e) {
      ...
      switch (v) {
        ...
        default:
          // check for defaults nodes for this slottype
          const nodeCreated = that.createDefaultNodeForSlot(
            Object.assign(opts, { position: [opts.e.canvasX, opts.e.canvasY], nodeType: v })
          )
          if (nodeCreated) {
            // new node created
    ...
    function refreshHelper() {
      ...
      if (that.onSearchBox) {
        ...
        function inner_test_filter(type, optsIn) {
          var optsIn = optsIn || {}
          const optsDef = { skipFilter: false, inTypeOverride: false, outTypeOverride: false }
          ...
          if ((!options.show_all_if_empty || str) && !type.toLowerCase().includes(str)) return false
  ...
  showEditPropertyValue(node, property, options) {
    ...
    if (type == 'string' || type == 'number' || type == 'array' || type == 'object') {
      ...
      for (const i in info.values) {
        ...
        input_html += `<option value='${v}' ${v == node.properties[property] ? 'selected' : ''}>${
          info.values[i]
        }</option>`
      }
      input_html += '</select>'
  ...
  showShowNodePanel(node) {
    ...
    function inner_refresh() {
      panel.content.innerHTML = '' // clear
      panel.addHTML(
        `<span class='node_type'>${node.type}</span><span class='node_desc'>${
      ...
      if (node.color !== undefined) {
        nodeCol = Object.keys(LGraphCanvas.node_colors).filter(nK => LGraphCanvas.node_colors[nK].color == node.color)
      }
    ...
    panel.inner_showCodePad = propname => {
      ...
      textarea.addEventListener('keydown', ({ code, ctrlKey }) => {
  ...
  showSubgraphPropertiesDialogRight(node) {
    ...
    const html =
      ' + <span class=\'label\'>Name</span><input class=\'name\'/><span class=\'label\'>Type</span><input class=\'type\'></input><button>+</button>'
    const elem = panel.addHTML(html, 'subgraph_property extra', true)
    elem.querySelector('.name').addEventListener('keydown', function ({ keyCode }) {
      if (keyCode == 13) {
  ...
  static onMenuNodeMode(value, options, e, menu, node) {
    new ContextMenu(LiteGraph.NODE_MODES, { event: e, callback: inner_clicked, parentMenu: menu, node })
  ...
  processContextMenu(node, event) {
    ...
    if (slot) {
      ...
      if (node.getSlotMenuOptions) {
        ...
        if (_slot.removable) {
          menu_info.push(_slot.locked ? 'Cannot remove' : { content: 'Remove Slot', slot })
        }
        if (!_slot.nameLocked) {
