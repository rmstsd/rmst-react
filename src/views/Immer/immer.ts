function immer<T extends object>(baseState: T, recipe: (draft: T) => void) {
  const copies = new Map() // 用于缓存被修改的对象, key 为原对象, value 为修改后的对象

  const proxies = new Map() // 用于存储被代理的对象, key 为原对象, value 为代理对象

  const proxy = createProxy(baseState)
  recipe(proxy)

  return finalize(baseState) as T
  ...
  function createProxy(base) {
    ...
    if (isPlainObject(base) || Array.isArray(base)) {
      ...
      if (!proxy) {
        const handler = {
          get(target, prop) {
            return createProxy(getCurrentSource(target)[prop]) // 增加一个 get 的劫持，返回一个 Proxy
          },
